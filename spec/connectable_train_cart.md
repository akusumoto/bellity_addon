...
| Inventory icon | `model_data/connectable_train_cart_icon.png`, deployed as `resource_pack/textures/items/connectable_train_cart.png` |
| Model orientation | Root geometry yaw offset of 90 degrees to align the cart body with rails |
...
The runtime geometry was found to be missing the editable model's `[0, 90, 0]` root-bone rotation, which caused the cart body to render sideways relative to rails. The rotation was restored, and validation now compares the complete runtime geometry with `model_data/connectable_train_cart.geo.json`. `node tools/validate.mjs`, `node --check behavior_pack/scripts/main.js`, and `git diff --check` passed on 2026-09-22. `install-dev.ps1 -Update` updated both development packs, and `tools/verify-dev.ps1` hash-verified 16 Behavior Pack files and 20 Resource Pack files. Correct alignment remains pending in-game checks on north-south and east-west straight rails, curves, and slopes for both heads and followers.

On 2026-09-23, the placement item was changed from the vanilla `minecart_normal` atlas entry to the supplied `model_data/connectable_train_cart_icon.png`. The Resource Pack copy is registered as `connectable_train_cart`, and validation checks the PNG, atlas mapping, item reference, and byte equality with the source asset. `node tools/validate.mjs`, `node --check behavior_pack/scripts/main.js`, and `git diff --check` passed. `install-dev.ps1 -Update` updated both development packs, and `tools/verify-dev.ps1` hash-verified 16 Behavior Pack files and 21 Resource Pack files. Correct icon rendering remains pending verification in Minecraft.
